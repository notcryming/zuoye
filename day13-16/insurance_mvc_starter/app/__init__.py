"""Flask 应用入口：create_app 工厂 + 建表/建admin/建Prompt模板 + 全局异常处理

【MVC 归属】基础设施层（应用工厂，串起 M/V/C 三层）
【思路】
1. _init_admin：建默认管理员 admin/admin123
2. _init_prompt_template：建默认 Prompt 模板（LLM 邮件生成用）
3. create_app：创建 app → 注册蓝图 → 注册异常处理器 → 建表+建 admin+建模板

【和主项目的关系】本文件和主项目 app/__init__.py 同名同职责，
学生 Day 3 切到主项目时，会看到主项目多了静态文件挂载，
...
def _init_prompt_template():
    """首次启动初始化默认 Prompt 模板至 prompt_templates 表

    1. 开独立 session（不在请求上下文里，手动管）
    2. 调 PromptTemplate.init_default_template（幂等：已有记录则跳过）
    3. finally 关闭 session
    """
    from app.models.prompt_template import PromptTemplate
    db = SessionLocal()
    try:
        PromptTemplate.init_default_template(db)
    finally:
        db.close()
...
def create_app() -> Flask:
    ...
    @app.route("/")
    def index():
        """前端 SPA 入口：直接返回 static/index.html"""
    ...
    with app.app_context():
        ...
        # 确保模型存储目录存在（训练产出的 .joblib 文件存放位置）
        os.makedirs(os.path.abspath(settings.MODEL_DIR), exist_ok=True)
