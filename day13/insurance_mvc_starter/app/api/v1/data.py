...
from app.utils.visualizer import render_chart
...
def upload():
    ...
    if size > MAX_UPLOAD_SIZE:
        raise BizException(1001, "文件过大，限制 10MB", 400)
...
def visualization(chart_type: str):
    """EDA 可视化：按 chart_type 画图，返回 {chart_type, image_base64, format:"png"}

    逐字思路：
    1. 查全表数据（无数据 → BizException 2001）
    2. render_chart 画图（未知 chart_type 内部抛 1001）
    3. 组装响应，base64 不带 data: 前缀（前端自行拼）
    """
    db = get_db()
